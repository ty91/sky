...
Slack과 Telegram에서 Claude Agent SDK 봇을 **CLI + 데몬**으로 다루는 프로젝트입니다.

## 특징

- Slack Assistant thread와 Telegram chat별로 Claude **long-lived query 세션**을 유지합니다.
...
- Slack에서는 같은 assistant thread, Telegram에서는 같은 채팅방 메시지를 같은 query 컨텍스트로 처리합니다.
- Telegram에서는 `/new` 명령으로 해당 채팅방 세션을 초기화할 수 있습니다.
- `~/.joy/settings.json`의 `workspace` 아래 `AGENTS.md`, `SOUL.md`, `USER.md`, `MEMORY.md`를 조립해 `systemPrompt`로 넣습니다.
...
```bash
cd ~/Developer/workspace/joy
pnpm install
```

`~/.joy/settings.json` 을 만듭니다:
...
```json
{
  ...
  "workspace": "/Users/taeyoung/.joy/workspace"
}
```
...
- `workspace` — 선택. 기본값 `~/.joy/workspace`. 이 디렉토리 아래의 `AGENTS.md`, `SOUL.md`, `USER.md`, `MEMORY.md`를 조립해 시스템 프롬프트로 사용합니다.
...
- `joy status`는 다음 정보를 보여줍니다.
  ...
  - 현재 backoff와 마지막 오류
- 런타임 health는 `~/.joy/runtime-health.json`에 저장됩니다.
- 데몬 PID/log 파일은 기본적으로 `~/.joy/` 아래에 저장됩니다.
- 세션 resume 매핑은 `~/.joy/sessions.json`에 저장됩니다.
