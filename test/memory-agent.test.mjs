...
test('memory agent processes unread transcripts and advances cursors', () => {
  const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'claudeclaw-memory-'));
...
const claudeclawDir = path.join(os.homedir(), '.claudeclaw');
const transcriptDir = path.join(claudeclawDir, 'transcripts', 'chat-1');
...
const cursors = JSON.parse(fs.readFileSync(path.join(claudeclawDir, 'memory-cursors.json'), 'utf8'));
